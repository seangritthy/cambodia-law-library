...
const html = fs.readFileSync('page_test.html', 'utf8');
...
$('a').each((i, el) => {
    ...
    if (href.toLowerCase().includes('.pdf')) {
        ...
        count++;
        pdfs.push({ title: text || 'Law ' + count, url: fullUrl, file: `law_l_${count}.pdf` });
    }
});
...
async function run() {
    ...
    for (let i = 0; i < pdfs.length; i++) {
        ...
        // Increased sleep to 3 seconds to avoid getting blocked
        await sleep(3000);
    }
